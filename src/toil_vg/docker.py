...
def dockerCall(job,
    ...
    _docker(job, tool=tool, parameters=parameters, workDir=workDir, dockerParameters=dockerParameters,
            outfile=outfile, errfile=errfile, checkOutput=False, defer=defer)
...
def dockerCheckOutput(job,
    ...
    return _docker(job, tool=tool, parameters=parameters, workDir=workDir,
                   dockerParameters=dockerParameters, checkOutput=True, errfile=errfile,
                   defer=defer)
...
def _docker(job,
    ...
    if outfile:
        subprocess.check_call(call, stdout=outfile, stderr=errfile)
    else:
        if checkOutput:
            return subprocess.check_output(call, stderr=errfile)
        else:
            subprocess.check_call(call, stderr=errfile)
