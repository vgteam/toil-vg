...
if [ $RUN_SMALL_TEST == false ]; then
    ...
    do
      ...
      if [ $(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*_R1*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | wc -l) -eq 1 ]; then
        ...
        PAIR_1_READS=()
        PAIR_2_READS=()
        LANE_NUMS=($(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | awk -F'_R' '{print $1}' | sort | uniq | xargs))
        for LANE_NUM in ${LANE_NUMS[@]}
        do
          PAIR_1_READS+=("$(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | grep "${LANE_NUM}.*_R1")")
          PAIR_2_READS+=("$(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | grep "${LANE_NUM}.*_R2")")
        done
      ...
      elif [ $(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*_1_*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | wc -l) -eq 1 ]; then
        ln -s $(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*_1_*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) ) ${READ_DATA_DIR}/${SAMPLE_NAME}_read_pair_1.fq.gz
        ln -s $(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*_2_*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) ) ${READ_DATA_DIR}/${SAMPLE_NAME}_read_pair_2.fq.gz
      elif [ $(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*_1_*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | wc -l) -gt 1 ]; then
        PAIR_1_READS=()
        PAIR_2_READS=()
        LANE_NUMS=($(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | awk -F'_' '{print $1"_"$2}' | sort | uniq | xargs))
        for LANE_NUM in ${LANE_NUMS[@]}
        do
          PAIR_1_READS+=("$(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | grep "${LANE_NUM}_1")")
          PAIR_2_READS+=("$(find ${INDIVIDUAL_DATA_DIR}/ \( \( \( -wholename '*.fastq.gz' \) -not -wholename '*WES*' \) -not -wholename '*WTS*' \) | grep "${LANE_NUM}_2")")
        done
        echo "INDIVIDUAL_DATA_DIR: ${INDIVIDUAL_DATA_DIR}"
        echo "LANE_NUMS: ${LANE_NUMS[@]}"
        echo "PAIR_1_READS: ${PAIR_1_READS[@]}"
